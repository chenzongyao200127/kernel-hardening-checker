...
def add_kconfig_checks(l: List[ChecklistObjType], arch: str) -> None:
    ...
    cc_is_clang = KconfigCheck(
        '-', '-', 'CC_IS_CLANG', 'y')  # exists since v4.18
    if arch in ('X86_64', 'X86_32'):
        cpu_sup_amd_not_set = KconfigCheck(
            '-', '-', 'CPU_SUP_AMD', 'is not set')
        cpu_sup_intel_not_set = KconfigCheck(
            '-', '-', 'CPU_SUP_INTEL', 'is not set')

    # radical, but may be useful in some cases
    modules_not_set = KconfigCheck(
        'cut_attack_surface', 'kspp', 'MODULES', 'is not set')
    devmem_not_set = KconfigCheck(
        'cut_attack_surface', 'kspp', 'DEVMEM', 'is not set')  # refers to LOCKDOWN
    bpf_syscall_not_set = KconfigCheck(
        'cut_attack_surface', 'lockdown', 'BPF_SYSCALL', 'is not set')  # refers to LOCKDOWN
    ...
    l += [KconfigCheck('self_protection', 'defconfig',
                       'THREAD_INFO_IN_TASK', 'y')]
    l += [KconfigCheck('self_protection', 'defconfig',
                       'IOMMU_DEFAULT_PASSTHROUGH', 'is not set')]
    # mutually exclusive with IOMMU_DEFAULT_DMA_STRICT
    iommu_support_is_set = KconfigCheck(
        'self_protection', 'defconfig', 'IOMMU_SUPPORT', 'y')
    l += [iommu_support_is_set]  # is needed for mitigating DMA attacks
    l += [OR(KconfigCheck('self_protection', 'defconfig', 'STACKPROTECTOR', 'y'),
             KconfigCheck('self_protection', 'defconfig',
                          'CC_STACKPROTECTOR', 'y'),
             KconfigCheck('self_protection', 'defconfig',
    ...
    l += [OR(KconfigCheck('self_protection', 'defconfig', 'STRICT_KERNEL_RWX', 'y'),
             # before v4.11
             KconfigCheck('self_protection', 'defconfig', 'DEBUG_RODATA', 'y'))]
    ...
    if arch in ('X86_64', 'ARM64', 'X86_32'):
        l += [KconfigCheck('self_protection', 'defconfig',
                           'RANDOMIZE_BASE', 'y')]
    vmap_stack_is_set = KconfigCheck(
        'self_protection', 'defconfig', 'VMAP_STACK', 'y')
    ...
    if arch in ('X86_64', 'X86_32'):
        ...
        microcode_is_set = KconfigCheck(
            'self_protection', 'defconfig', 'MICROCODE', 'y')
        ...
        l += [OR(KconfigCheck('self_protection', 'defconfig', 'MICROCODE_INTEL', 'y'),
                 ...
                 AND(microcode_is_set,
                     # MICROCODE_INTEL was included in MICROCODE since v6.6
                     VersionCheck((6, 6, 0))))]
        l += [OR(KconfigCheck('self_protection', 'defconfig', 'MICROCODE_AMD', 'y'),
                 cpu_sup_amd_not_set,
                 AND(microcode_is_set,
                     # MICROCODE_AMD was included in MICROCODE since v6.6
                     VersionCheck((6, 6, 0))))]
    ...
    l += [KconfigCheck('self_protection', 'kspp',
    ...
    cfi_clang_is_set = KconfigCheck(
        'self_protection', 'kspp', 'CFI_CLANG', 'y')
    cfi_clang_permissive_not_set = KconfigCheck(
        'self_protection', 'kspp', 'CFI_PERMISSIVE', 'is not set')
    l += [OR(KconfigCheck('self_protection', 'kspp', 'DEBUG_CREDENTIALS', 'y'),
             # DEBUG_CREDENTIALS was dropped in v6.6.8
             VersionCheck((6, 6, 8)))]
...
#             randstruct_is_set)] # comment this out for now: KSPP has revoked this recommendation
    hardened_usercopy_is_set = KconfigCheck(
    ...
    l += [AND(KconfigCheck('self_protection', 'kspp', 'HARDENED_USERCOPY_PAGESPAN', 'is not set'),
              # this debugging for HARDENED_USERCOPY is not needed for security
    ...
    l += [OR(KconfigCheck('self_protection', 'kspp', 'MODULE_SIG_SHA512', 'y'),
             KconfigCheck('self_protection', 'a13xp0p0v',
                          'MODULE_SIG_SHA3_512', 'y'),
             modules_not_set)]
    ...
    ubsan_bounds_is_set = KconfigCheck(
        'self_protection', 'kspp', 'UBSAN_BOUNDS', 'y')
    ...
    l += [AND(KconfigCheck('self_protection', 'kspp', 'UBSAN_TRAP', 'y'),
              ubsan_bounds_is_set,
              KconfigCheck('self_protection', 'kspp',
                           'UBSAN_SHIFT', 'is not set'),
              KconfigCheck('self_protection', 'kspp',
                           'UBSAN_DIV_ZERO', 'is not set'),
              KconfigCheck('self_protection', 'kspp',
                           'UBSAN_UNREACHABLE', 'is not set'),
              KconfigCheck('self_protection', 'kspp',
                           'UBSAN_SIGNED_WRAP', 'is not set'),
              KconfigCheck('self_protection', 'kspp',
                           'UBSAN_BOOL', 'is not set'),
              KconfigCheck('self_protection', 'kspp',
                           'UBSAN_ENUM', 'is not set'),
    ...
    if arch in ('X86_64', 'ARM64', 'X86_32'):
        stackleak_is_set = KconfigCheck(
            'self_protection', 'kspp', 'GCC_PLUGIN_STACKLEAK', 'y')
    ...
    if arch in ('ARM64', 'ARM', 'RISCV'):
        # another reason?
        l += [KconfigCheck('self_protection', 'kspp', 'SYN_COOKIES', 'y')]
    ...
    if arch == 'ARM':
        ...
        l += [KconfigCheck('self_protection', 'a13xp0p0v',
                           'ARM_SMMU_DISABLE_BYPASS_BY_DEFAULT', 'y')]
    ...
    l += [KconfigCheck('security_policy', 'kspp',
                       'SECURITY_SELINUX_DISABLE', 'is not set')]
    l += [KconfigCheck('security_policy', 'kspp',
    ...
    l += [KconfigCheck('security_policy', 'kspp',
                       'SECURITY_WRITABLE_HOOKS', 'is not set')]
    l += [KconfigCheck('security_policy', 'kspp',
                       'SECURITY_SELINUX_DEBUG', 'is not set')]
    l += [OR(KconfigCheck('security_policy', 'a13xp0p0v', 'SECURITY_SELINUX', 'y'),
             KconfigCheck('security_policy', 'a13xp0p0v',
                          'SECURITY_APPARMOR', 'y'),
             KconfigCheck('security_policy', 'a13xp0p0v',
                          'SECURITY_SMACK', 'y'),
    ...
    l += [KconfigCheck('cut_attack_surface', 'defconfig',
                       'SECCOMP_FILTER', 'y')]
    ...
    l += [KconfigCheck('cut_attack_surface', 'kspp',
                       'SECURITY_DMESG_RESTRICT', 'y')]
    l += [KconfigCheck('cut_attack_surface', 'kspp',
                       'ACPI_CUSTOM_METHOD', 'is not set')]  # refers to LOCKDOWN
    l += [KconfigCheck('cut_attack_surface', 'kspp',
                       'COMPAT_BRK', 'is not set')]
    # refers to LOCKDOWN
    l += [KconfigCheck('cut_attack_surface', 'kspp', 'DEVKMEM', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'kspp',
                       'BINFMT_MISC', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'kspp',
                       'INET_DIAG', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'kspp', 'KEXEC',
                       'is not set')]  # refers to LOCKDOWN
    # refers to LOCKDOWN
    l += [KconfigCheck('cut_attack_surface', 'kspp',
                       'PROC_KCORE', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'kspp',
    ...
    l += [KconfigCheck('cut_attack_surface', 'kspp',
                       'IA32_EMULATION', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'kspp', 'X86_X32', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'kspp',
                       'X86_X32_ABI', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'kspp',
                       'MODIFY_LDT_SYSCALL', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'kspp',
                       'OABI_COMPAT', 'is not set')]
    # refers to LOCKDOWN
    l += [KconfigCheck('cut_attack_surface', 'kspp', 'X86_MSR', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'kspp',
                       'LEGACY_TIOCSTI', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'kspp',
                       'MODULE_FORCE_LOAD', 'is not set')]
    l += [modules_not_set]
    l += [devmem_not_set]
    ...
    l += [KconfigCheck('cut_attack_surface', 'maintainer',
                       'DRM_LEGACY', 'is not set')]
    ...
    # recommended by Daniel Vetter in /issues/38
    l += [KconfigCheck('cut_attack_surface', 'maintainer',
                       'BLK_DEV_FD', 'is not set')]
    # recommended by Denis Efremov in /pull/54
    l += [KconfigCheck('cut_attack_surface', 'maintainer',
                       'BLK_DEV_FD_RAWCMD', 'is not set')]
    # recommended by Denis Efremov in /pull/62
    l += [KconfigCheck('cut_attack_surface', 'maintainer',
                       'NOUVEAU_LEGACY_CTX_SUPPORT', 'is not set')]
    ...
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'ZSMALLOC_STAT', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'DEBUG_KMEMLEAK', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'BINFMT_AOUT', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'KPROBE_EVENTS', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'UPROBE_EVENTS', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'GENERIC_TRACER', 'is not set')]  # refers to LOCKDOWN
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'FUNCTION_TRACER', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'STACK_TRACER', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'HIST_TRIGGERS', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'BLK_DEV_IO_TRACE', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
    ...
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'CHECKPOINT_RESTORE', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'USERFAULTFD', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'HWPOISON_INJECT', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'MEM_SOFT_DIRTY', 'is not set')]
    # refers to LOCKDOWN
    l += [KconfigCheck('cut_attack_surface', 'grsec', 'DEVPORT', 'is not set')]
    # refers to LOCKDOWN
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'DEBUG_FS', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'NOTIFIER_ERROR_INJECTION', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'FAIL_FUTEX', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'PUNIT_ATOM_DEBUG', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'ACPI_CONFIGFS', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'EDAC_DEBUG', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'DRM_I915_DEBUG', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'DVB_C8SECTPFE', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'MTD_SLRAM', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'MTD_PHRAM', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'IO_URING', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec', 'KCMP', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec', 'RSEQ', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'LATENCYTOP', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec', 'KCOV', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'PROVIDE_OHCI1394_DMA_INIT', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'SUNRPC_DEBUG', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'X86_16BIT', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'BLK_DEV_UBLK', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'SMB_SERVER', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'XFS_ONLINE_SCRUB_STATS', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'CACHESTAT_SYSCALL', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'PREEMPTIRQ_TRACEPOINTS', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'ENABLE_DEFAULT_TRACERS', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'PROVE_LOCKING', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'TEST_DEBUG_VIRTUAL', 'is not set')]
    ...
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'PTDUMP_DEBUGFS', 'is not set')]
    # the old name of PTDUMP_DEBUGFS
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'X86_PTDUMP', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'DEBUG_CLOSURES', 'is not set')]
    # the old name of DEBUG_CLOSURES
    l += [KconfigCheck('cut_attack_surface', 'grsec',
                       'BCACHE_CLOSURES_DEBUG', 'is not set')]

    # 'cut_attack_surface', 'clipos'
    l += [KconfigCheck('cut_attack_surface', 'clipos',
                       'STAGING', 'is not set')]
    # to prevent FLUSH+RELOAD attack
    l += [KconfigCheck('cut_attack_surface', 'clipos', 'KSM', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'clipos',
                       'KALLSYMS', 'is not set')]
    ...
    l += [KconfigCheck('cut_attack_surface', 'clipos',
                       'CRASH_DUMP', 'is not set')]
    # user.max_user_namespaces=0
    l += [KconfigCheck('cut_attack_surface', 'clipos',
                       'USER_NS', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'clipos',
                       'X86_CPUID', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'clipos',
                       'X86_IOPL_IOPERM', 'is not set')]  # refers to LOCKDOWN
    l += [KconfigCheck('cut_attack_surface', 'clipos',
                       'ACPI_TABLE_UPGRADE', 'is not set')]  # refers to LOCKDOWN
    l += [KconfigCheck('cut_attack_surface', 'clipos',
                       'EFI_CUSTOM_SSDT_OVERLAYS', 'is not set')]
...
#   l += [KconfigCheck('cut_attack_surface', 'clipos', 'IKCONFIG', 'is not set')] # no, IKCONFIG is needed for this check :)
    ...
    l += [KconfigCheck('cut_attack_surface', 'lockdown',
                       'EFI_TEST', 'is not set')]  # refers to LOCKDOWN
    l += [KconfigCheck('cut_attack_surface', 'lockdown',
                       'MMIOTRACE_TEST', 'is not set')]  # refers to LOCKDOWN
    l += [KconfigCheck('cut_attack_surface', 'lockdown',
                       'KPROBES', 'is not set')]  # refers to LOCKDOWN
    l += [bpf_syscall_not_set]  # refers to LOCKDOWN

    # 'cut_attack_surface', 'a13xp0p0v'
    # refers to LOCKDOWN (permissive)
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'MMIOTRACE', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'LIVEPATCH', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'IP_DCCP', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'FTRACE', 'is not set')]  # refers to LOCKDOWN
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'VIDEO_VIVID', 'is not set')]
    # can be used as a keylogger
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'INPUT_EVBUG', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'CORESIGHT', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'XFS_SUPPORT_V4', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'BLK_DEV_WRITE_MOUNTED', 'is not set')]
    # see the comment about bdev_allow_write_mounted below
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'FAULT_INJECTION', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'ARM_PTDUMP_DEBUGFS', 'is not set')]
    # the old name of ARM_PTDUMP_DEBUGFS
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'ARM_PTDUMP', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'SECCOMP_CACHE_DEBUG', 'is not set')]
    l += [KconfigCheck('cut_attack_surface', 'a13xp0p0v',
                       'LKDTM', 'is not set')]
    ...
    # 'harden_userspace'
    if arch == 'ARM64':
        l += [KconfigCheck('harden_userspace', 'defconfig',
                           'ARM64_PTR_AUTH', 'y')]
    ...
    l += [KconfigCheck('harden_userspace', 'a13xp0p0v',
                       'ARCH_MMAP_RND_BITS', 'MAX')]
    ...
    if arch == 'X86_64':
        l += [KconfigCheck('harden_userspace', 'kspp',
...
def add_cmdline_checks(l: List[ChecklistObjType], arch: str) -> None:
    ...
    l += [CmdlineCheck('self_protection', 'defconfig',
                       'nokaslr', 'is not set')]
    l += [CmdlineCheck('self_protection', 'defconfig', 'nopti', 'is not set')]
    l += [CmdlineCheck('self_protection', 'defconfig',
                       'no_hash_pointers', 'is not set')]
    l += [CmdlineCheck('self_protection', 'defconfig',
                       'nospectre_v1', 'is not set')]
    l += [CmdlineCheck('self_protection', 'defconfig',
                       'nospectre_v2', 'is not set')]
    l += [CmdlineCheck('self_protection', 'defconfig',
                       'nospectre_bhb', 'is not set')]
    l += [CmdlineCheck('self_protection', 'defconfig',
                       'nospec_store_bypass_disable', 'is not set')]
    l += [CmdlineCheck('self_protection', 'defconfig',
                       'dis_ucode_ldr', 'is not set')]
    l += [CmdlineCheck('self_protection', 'defconfig',
                       'arm64.nobti', 'is not set')]
    l += [CmdlineCheck('self_protection', 'defconfig',
                       'arm64.nopauth', 'is not set')]
    l += [CmdlineCheck('self_protection', 'defconfig',
                       'arm64.nomte', 'is not set')]
    ...
    if arch in ('X86_64', 'X86_32'):
        ...
        l += [OR(CmdlineCheck('self_protection', 'defconfig', 'spectre_bhi', 'is not off'),
                 AND(KconfigCheck('self_protection', 'defconfig', 'MITIGATION_SPECTRE_BHI', 'y'),
                     CmdlineCheck('self_protection', 'kspp',
    ...
    if arch == 'ARM64':
        ...
        l += [OR(CmdlineCheck('self_protection', 'defconfig', 'ssbd', 'kernel'),
                 CmdlineCheck('self_protection', 'a13xp0p0v',
                              'ssbd', 'force-on'),
                 AND(CmdlineCheck('self_protection', 'defconfig', 'mitigations', 'auto'),
    ...
    # 'self_protection', 'kspp'
    # consequence of 'slab_nomerge' by kspp
    l += [CmdlineCheck('self_protection', 'kspp', 'slab_merge', 'is not set')]
    # consequence of 'slab_nomerge' by kspp
    l += [CmdlineCheck('self_protection', 'kspp', 'slub_merge', 'is not set')]
    ...
    if arch in ('X86_64', 'X86_32'):
        l += [CmdlineCheck('self_protection', 'kspp',
                           'mitigations', 'auto,nosmt')]
    ...
    # 'cut_attack_surface', 'defconfig'
    if arch in ('X86_64', 'X86_32'):
        tsx_not_set = CmdlineCheck(
            'cut_attack_surface', 'defconfig', 'tsx', 'is not set')
    ...
    # 'cut_attack_surface', 'kspp'
    # slow (high performance penalty)
    l += [CmdlineCheck('cut_attack_surface', 'kspp', 'nosmt', 'is present')]
    if arch == 'X86_64':
        l += [OR(CmdlineCheck('cut_attack_surface', 'kspp', 'vsyscall', 'none'),
                 KconfigCheck('cut_attack_surface', 'kspp',
    ...
    # 'cut_attack_surface', 'grapheneos'
    l += [CmdlineCheck('cut_attack_surface', 'grapheneos',
                       'sysrq_always_enabled', 'is not set')]
    ...
    # 'harden_userspace'
    l += [CmdlineCheck('harden_userspace', 'defconfig',
                       'norandmaps', 'is not set')]
...
def add_sysctl_checks(l: List[ChecklistObjType], arch: StrOrNone) -> None:
    ...
    l += [SysctlCheck('self_protection', 'a13xp0p0v',
                      'kernel.oops_limit', '100')]
    l += [SysctlCheck('self_protection', 'a13xp0p0v',
                      'kernel.warn_limit', '100')]
    # Compatible with the 'DEFAULT_MMAP_MIN_ADDR' kconfig check by KSPP:
    if arch in ('X86_64', 'X86_32', 'RISCV'):
        l += [SysctlCheck('self_protection', 'kspp',
    ...
    l += [SysctlCheck('cut_attack_surface', 'kspp',
    ...
    l += [SysctlCheck('harden_userspace', 'kspp',
    ...
    l += [SysctlCheck('harden_userspace', 'a13xp0p0v',
