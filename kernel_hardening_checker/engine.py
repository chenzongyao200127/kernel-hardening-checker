...
def colorize_result(input_text: StrOrNone) -> StrOrNone:
    ...
    if input_text.startswith('OK'):
        ...
        assert (input_text.startswith('FAIL:')
                ), f'unexpected result "{input_text}"'
        color = RED_COLOR
...
class ComplexOptCheck:
    ...
    @property
    def name(self) -> str:
        # true for SimpleNamedOptCheckTypes
        assert hasattr(self.opts[0], 'name')
        return self.opts[0].name

    @property
    def expected(self) -> str:
        # true for SimpleNamedOptCheckTypes
        assert hasattr(self.opts[0], 'expected')
    ...
    def json_dump(self, with_results: bool) -> Dict[str, StrOrBool]:
        # true for SimpleNamedOptCheckTypes
        assert hasattr(self.opts[0], 'json_dump')
...
SimpleOptCheckType = Union[KconfigCheck,
                           CmdlineCheck, SysctlCheck, VersionCheck]
...
AnyOptCheckType = Union[KconfigCheck, CmdlineCheck,
...
def populate_simple_opt_with_data(opt: SimpleOptCheckType, data: DictOrTuple, data_type: str) -> None:
    assert (opt.opt_type !=
            'complex'), f'unexpected opt_type "{opt.opt_type}" for {opt}'
    assert (
        opt.opt_type in SIMPLE_OPTION_TYPES), f'invalid opt_type "{opt.opt_type}"'
    assert (
        data_type in SIMPLE_OPTION_TYPES), f'invalid data_type "{data_type}"'
...
def print_unknown_options(checklist: List[ChecklistObjType], parsed_options: Dict[str, str], opt_type: str) -> None:
    ...
    for o1 in checklist:
        if isinstance(o1, SimpleOptCheckTypes):
            ...
            assert (not isinstance(o1, VersionCheck)
                    ), 'single VersionCheck in checklist'
            known_options.append(o1.name)
            continue
        for o2 in o1.opts:
            if isinstance(o2, SimpleOptCheckTypes):
                assert (o2.opt_type !=
                        'complex'), f'{o2} with complex opt_type'
                if hasattr(o2, 'name'):
            ...
            for o3 in o2.opts:
                ...
                assert (o3.opt_type !=
                        'complex'), f'{o3} with complex opt_type'
                if hasattr(o3, 'name'):
